                ...
                <div class="shadow border-b border-gray-200 sm:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <thead class="bg-gray-50">
                            <tr>
                                <th v-if="mutipleCheck" class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"><input type="checkbox" class="w-3 h-3" v-model="selectAllItem" @click="selectItem" /></th>
                                <th v-for="(item, index) in columns" :key="index" scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">
                                    <div class="flex items-center">
                                        <div>{{ item.label }}</div>
                                        <button v-if="sortData && item.sort" @click="applySort(item.field)" class="focus:outline-none">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="(item, index) in data" :key="index" class="text-sm font-medium">
                                <td v-if="mutipleCheck" class="px-6 py-3 whitespace-nowrap"><input type="checkbox" class="w-3 h-3" :value="item" v-model="selectedItem" @change="sendSelectedData" /></td>
                                <template v-for="col in columns">
                                    <td v-if="typeof $scopedSlots[col.field] !== 'undefined'" :key="col.id" class="px-6 py-3 whitespace-nowrap">
